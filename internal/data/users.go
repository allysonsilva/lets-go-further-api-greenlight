...
func (m UserModel) Insert(user *User) error {
	...
	regexMessage := regexp.MustCompile(`Duplicate entry '(?:.*)' for key 'users.email_unique_idx'$`)

	if err != nil {
		switch {
		case regexMessage.MatchString(err.Error()):
	...
	err = m.DB.QueryRowContext(ctx, `SELECT id, created_at, version FROM users WHERE id = ?`, lId).Scan(&user.ID, &user.CreatedAt, &user.Version)
...
func (m UserModel) Update(user *User) error {
	...
	err = m.DB.QueryRowContext(ctx, `SELECT version FROM users WHERE id = ?`, user.ID).Scan(&user.Version)
...
func (m UserModel) GetForToken(tokenScope, tokenPlaintext string) (*User, error) {
	...
	args := []interface{}{hex.EncodeToString(tokenHash[:]), tokenScope, time.Now()}
