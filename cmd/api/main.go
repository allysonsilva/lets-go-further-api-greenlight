...
import (
	...
	"greenlight.allyson.dev/internal/jsonlog"
	"greenlight.allyson.dev/internal/mailer"
)
...
func main() {
	...
	flag.StringVar(&cfg.db.dsn, "db-dsn", "user:pass@tcp(localhost:3306)/greenlight?parseTime=true&tls=skip-verify", "MySQL DSN")
	...
	flag.StringVar(&cfg.smtp.sender, "smtp-sender", "Greenlight <no-reply@greenlight.allyson.dev>", "SMTP sender")
	...
	//
	// It’s important to point out here that the interface{} value returned
	// from this function must encode to JSON without any errors.
	// If it can’t be encoded to JSON, then it will be omitted from the
	// expvar output and the response from the GET /debug/vars endpoint
	// will be malformed. Any error will be silently discarded.
